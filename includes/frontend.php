...
function push_notification_enqueue_scripts() {
    wp_enqueue_script('push-notification-js', plugin_dir_url(dirname(__DIR__) . '/push-notification-integration.php') . 'js/push-notification.js', array('jquery'), '1.0.0', true);
...
function push_notification_add_service_worker() {
    echo '<script>
        if ("serviceWorker" in navigator) {
            navigator.serviceWorker.register("' . plugin_dir_url(dirname(__DIR__) . '/push-notification-integration.php') . 'sw.js")
